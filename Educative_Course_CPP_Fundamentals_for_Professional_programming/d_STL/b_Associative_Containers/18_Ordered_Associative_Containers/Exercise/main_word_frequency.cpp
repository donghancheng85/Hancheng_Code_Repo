...
int main(int argc,  char* argv[]){
  ...
  std::cout << "All word which appears more then 3 times:" << std::endl;
  auto biggerIt= std::find_if(allFreq.begin(), allFreq.end(), [](intAndWords iAndW){return iAndW.first > 3;});
  if (biggerIt == allFreq.end()){
    std::cerr << "No word appears more then 3 times !" << std::endl;
    exit(EXIT_FAILURE);
  }
  else{
